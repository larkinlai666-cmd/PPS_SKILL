...
- Every session anchors the objective: `session_begin` hashes the objective-bearing sections into `.pps/objective-anchor`. A mid-session objective change fails the gate unless `EVENTS.md` records an `objective-revised` event. The gate also prints the objective, red lines, and active decisions on every run, as an operator log line — not as a memory mechanism.
