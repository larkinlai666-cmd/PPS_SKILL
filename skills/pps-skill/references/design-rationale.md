...
The objective anchor and acceptance fields are one such distillation. External workflows fight context rot with fresh-context subagents and fight goal drift with generated specs — both runtime-shaped mechanisms PPS cannot host. PPS redefines the parts it can enforce as protocol invariants: `session_begin` hashes the goal-bearing sections into `.pps/objective-anchor`, the gate compares against it and refuses an unrecorded rewrite, and `Acceptance` binds each item of "done" to a check the gate proves ran. What PPS deliberately does not claim is parity on working memory: the gate runs at close, so it cannot un-rot a session that already drifted. That half is handled by re-running the bounded resume packet mid-session, and by keeping the anchored objective readable inside the anchor file. The principle is adopted where a deterministic script can carry it; where it cannot, the limit is documented instead of advertised away.
