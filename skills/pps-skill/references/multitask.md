...
- Every non-empty disposition set carries its own evidence, independent of the total `Status`: a non-empty `Rejected` set requires a `Reason`, and a non-empty `Deferred` set requires a `Reactivate When` — on ANY receipt, not only all-rejected or all-deferred ones. A rejection without a reason and a deferral without a reactivation condition are silent abandonments.
- `Status: integrated` must not mask still-open dispositions: a receipt that lists non-empty `Rejected` or `Deferred` paths while claiming `integrated` compresses unfinished lifecycles into one word. Split mixed dispositions into separate receipts (one per independently decidable output unit) or declare `Status: partially_integrated`, which requires a non-empty `Accepted` set AND at least one non-empty `Rejected`/`Deferred` set (with their evidence above). A task with a `partially_integrated` receipt stays `active`/`handoff_ready` in `TASK_INDEX.md` until the remainder is resolved by further receipts; the registry must never claim it `integrated`, `deferred`, `rejected`, or `archived`.
- Every terminal disposition (`integrated`/`partially_integrated`/`deferred`/`rejected`) requires Approval and Verification.
